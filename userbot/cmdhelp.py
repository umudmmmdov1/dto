...
class CmdHelp:
    ...
    def get_result(self):
        ...
        result = f"**📗 Fayl/File:** `{self.FILE}`\n"
        if self.WARNING == '' and self.INFO == '':
            result += f"**♻️ Rəsmi/Official:** {'✅' if self.IS_OFFICIAL else '❌'}\n\n"
        else:
            result += f"**♻️ Rəsmi/Official:** {'✅' if self.IS_OFFICIAL else '❌'}\n"
            
            if self.INFO == '':
                if not self.WARNING == '':
                    result += f"**❗ Xəbərdarlıq/Warning:** {self.WARNING}\n\n"
            else:
                if not self.WARNING == '':
                    result += f"**❗ Xəbərdarlıq/Warning:** {self.WARNING}\n"
                result += f"**ℹ️ Məlumat/Info:** {self.INFO}\n\n"
                     
        for command in self.COMMANDS:
            command = self.COMMANDS[command]
            if command['params'] == None:
                result += f"**⌨️ Əmr/Command:** `{PATTERNS[:1]}{command['command']}`\n"
            else:
                result += f"**⌨️ Əmr/Command:** `{PATTERNS[:1]}{command['command']} {command['params']}`\n"
                
            if command['example'] == None:
                result += f"**📃 Açıqlama/Disclosure:** `{command['usage']}`\n\n"
            else:
                result += f"**📃 Açıqlama/Disclosure:** `{command['usage']}`\n"
                result += f"**➤ Məsələn/Example:** `{PATTERNS[:1]}{command['example']}`\n\n"
        return result
